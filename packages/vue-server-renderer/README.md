...
> This package is auto-generated. For pull requests please see [src/entries/web-server-renderer.js](https://github.com/blujs/blu/blob/dev/src/entries/web-server-renderer.js).

This package offers Node.js server-side rendering for Blu 2.0.
...
``` bash
npm install blu-server-renderer
```
...
``` js
const renderer = require('blu-server-renderer').createRenderer()
```
...
Render a Blu instance to string. The callback is a standard Node.js callback that receives the error as the first argument:
...
Render a Blu instance in streaming mode. Returns a Node.js readable stream.
...
``` js
const bundleRenderer = require('blu-server-renderer').createBundleRenderer(code)
```
...
As an example, check out [`v-show`'s server-side implementation](https://github.com/blujs/blu/blob/dev/src/platforms/web/server/directives/show.js).
...
The application bundle can be generated by any build tool, so you can easily use Webpack + `blu-loader` with the bundleRenderer. You do need to use a slightly different webpack config and entry point for your server-side bundle, but the difference is rather minimal:
...
2. In your server-side entry point, export a function. The function will receive the render context object (passed to `bundleRenderer.renderToString` or `bundleRenderer.renderToStream`), and should return a Promise, which should eventually resolve to the app's root Blu instance:

  ``` js
  // server-entry.js
  import Blu from 'blu'
  import App from './App.blu'

  const app = new Blu(App)
...
Note that cachable component **must also define a unique "name" option**. This is necessary for Blu to determine the identity of the component when using the
bundle renderer.
...
In server-rendered output, the root element will have the `server-rendered="true"` attribute. On the client, when you mount a Blu instance to an element with this attribute, it will attempt to "hydrate" the existing DOM instead of creating new DOM nodes.

In development mode, Blu will assert the client-side generated virtual DOM tree matches the DOM structure rendered from the server. If there is a mismatch, it will bail hydration, discard existing DOM and render from scratch. **In production mode, this assertion is disabled for maximum performance.**

### Hydration Caveats

One thing to be aware of when using SSR + client hydration is some special HTML structures that may be altered by the browser. For example, when you write this in a Blu template:
...
The browser will automatically inject `<tbody>` inside `<table>`, however, the virtual DOM generated by Blu does not contain `<tbody>`, so it will cause a mismatch. To ensure correct matching, make sure to write valid HTML in your templates.
