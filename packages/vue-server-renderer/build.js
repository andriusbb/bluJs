...
if (process.env.NODE_ENV !== 'production') {
  ...
  formatComponentName = function (vm$$1) {
    ...
    var name = vm$$1._isBlu
      ? vm$$1.$options.name || vm$$1.$options._componentTag
      : vm$$1.name;
    return (
      (name ? ("component <" + name + ">") : "anonymous component") +
      (vm$$1._isBlu && vm$$1.$options.__file ? (" at " + (vm$$1.$options.__file)) : '')
    )
  };
...
function initEvents (vm$$1) {
  ...
  vm$$1._updateListeners = function (listeners, oldListeners) {
    updateListeners(listeners, oldListeners || {}, on, off, vm$$1);
  };
  if (listeners) {
  ...
  function renderNode (
    ...
    if (node.componentOptions) {
      ...
      if (getKey && cache && name) {
        ...
        if (getKey && !cache) {
          warnOnce(
            "[blu-server-renderer] Component " + (Ctor.options.name || '(anonymous)') + " implemented serverCacheKey, " +
